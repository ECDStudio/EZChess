...
describe('availableMoves', function() {
  ...
  it('should return an empty array', function() {
    expect(_king.availableMoves()).toEqual([]);
  });
  it('should return a list of 1 positions', function() {
    chess.players.player1.pieces.bishop1.toPosition(-1, -1);
    expect(_king.availableMoves().length).toEqual(1);
  });
  it('should return a list of 8 positions', function() {
    _king.toPosition(3, 4);
    expect(_king.availableMoves().length).toEqual(8);
  });
  it('should return a list of 5 positions', function() {
    _king.toPosition(4, 1);
    expect(_king.availableMoves().length).toEqual(5);
  });

  describe('King Side Castling Scenerio:', function() {
    const clearPath = () => {
      chess.players.player1.pieces.knight1.toPosition(-1, -1);
      chess.players.player1.pieces.bishop1.toPosition(-1, -1);
    }

    it('should return a list of 2 positions', function() {
      clearPath();
      expect(_king.availableMoves().length).toEqual(2);
    });
    it('rook should be at position [0, 2]', function() {
      clearPath();
      _king.toPosition(0, 1, 'king-castle');
    ...
    it('enemy queen marking the path to castle', function() {
      clearPath();
      chess.players.player1.pieces.pawn2.toPosition(-1, -1);
      chess.players.player2.pieces.queen.toPosition(3, 1);
      expect(_king.availableMoves().length).toEqual(1);
    });
    it('enemy knight marking the path to castle', function() {
      clearPath();
      chess.players.player2.pieces.knight1.toPosition(2, 1);
      expect(_king.availableMoves().length).toEqual(0);
    });
    it('king is in check', function() {
      clearPath();
      chess.players.player2.pieces.knight1.toPosition(2, 4);
      expect(_king.availableMoves().length).toEqual(1);
    });
  });
  ...
  describe('Queen Side Castling Scenerio:', function() {
    const clearPath = () => {
      chess.players.player1.pieces.queen.toPosition(-1, -1);
      chess.players.player1.pieces.knight2.toPosition(-1, -1);
      chess.players.player1.pieces.bishop2.toPosition(-1, -1);
    }
    ...
    it('rook should be at position [0, 4]', function() {
      clearPath();
      _king.toPosition(0, 5, 'queen-castle');
    ...
    it('enemy queen marking the path to castle', function() {
      clearPath();
      chess.players.player1.pieces.pawn5.toPosition(-1, -1);
      chess.players.player2.pieces.queen.toPosition(3, 4);
      expect(_king.availableMoves().length).toEqual(0);
    });
    it('enemy knight marking the path to castle', function() {
      clearPath();
      chess.players.player2.pieces.knight1.toPosition(2, 3);
      expect(_king.availableMoves().length).toEqual(0);
    });
